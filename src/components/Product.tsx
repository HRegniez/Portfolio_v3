  ...
  return (
    <div className="py-10">
      ...
      <div className="flex flex-row justify-center my-8 flex-wrap">
        {product.images && product.images.length > 1 && product.images.map((image, idx) => (
          <button
      ...
      <div className="flex lg:flex-row justify-between items-center flex-col mt-20">
        ...
        <div className="flex space-x-2 md:mb-1 mt-2 md:mt-0">
          {product.stack?.map((stack: string) => (
            <span
              key={stack}
              className="text-xs  md:text-xs lg:text-xs bg-gray-200 px-2 py-1 rounded-sm text-primary"
            >
              {stack}
            </span>
          ))}
      ...
      <div>
        <Paragraph className="max-w-xl mt-4 text-neutral-700">{product.description}</Paragraph>
      </div>
      <div className="prose prose-sm md:prose-base max-w-none text-neutral-500">
        {product?.content}
      </div>
      <div className="flex w-full mt-20 justify-between flex-row gap-2">
        <Link
          href={product.githubLink}
          target="__blank"
        className="inline-flex items-center gap-1 group/button rounded-sm hover:scale-105 focus:outline-none transition ring-offset-gray-900 bg-gray-200 text-primary shadow-lg shadow-black/20 sm:backdrop-blur-sm group-hover/button:bg-gray-50/15 group-hover/button:scale-105 focus-visible:ring-1 focus-visible:ring-offset-2 ring-gray-50/60 text-sm font-medium px-4 py-2 mt-auto origin-left"
      >
        View code
      </Link>
      <Link
